 ...
 * Daniel JosÃ© Ojeda Loisel
 * Juan David Vega RodrÃ­guez
 * Miguel Ãngel Viera GonzÃ¡lez
 */
...
public:
	...
	TestCase(int id, const string &input, const vector<string> &output,
		    ...
		    string failMessage, string programToRun, string programArgs,
                    int expectedExitCode, string group);
...
void Evaluation::addTestCase(Case &caso) {
	...
	testCases.push_back(TestCase(testCases.size() + 1, caso.getInput(), caso.getOutput(),
			...
			caso.getProgramToRun(), caso.getProgramArgs(),
                        caso.getExpectedExitCode(), caso.getGroup()) );
}
	...
	for (int i = 0; i < nlines; i++) {
		...
		if (state == regular && tag.size()) {
			if (tag == INPUT_TAG) {
				...
				group = Tools::toLower(Tools::trim(value));
				caso.setGroup(group);
...
void Evaluation::runTests() {
	...
	set<string> ngroups, failedGroup;
	bool groups = false;
	for (size_t i = 0; i < testCases.size(); i++) {
		ngroups.insert( testCases[i].getGroup() );
	}
	nerrors = 0;
	nruns = 0;
	nskipped = 0;
	...
	if (ngroups.size() > 1 || testCases[0].getGroup() != "") {
		groups = true;
		defaultGradeReduction = (grademax - grademin) / ngroups.size();
	}
	
	int timeout = maxtime / testCases.size();
	for (size_t i = 0; i < testCases.size(); i++) {
		if ( groups && failedGroup.count(testCases[i].getGroup()) != 0) {
		    nskipped++;
			continue;
		}
		...
		if (!testCases[i].isCorrectResult()) {
			...
			nerrors++;
			if ( groups ) {
				failedGroup.insert(testCases[i].getGroup());
			}
			if(ncomments<MAXCOMMENTS){
	...
	if (testCases.size() > 0) {
		...
		if ( nruns > 0 ) {
			...
			if ( nskipped > 0 ) {
				...
				printf(">| %2d %s run/%2d %s passed/%2d %s skipped|\n",
					nruns, nruns==1?stest[0]:stest[1],
					passed, passed==1?stest[0]:stest[1],
					nskipped, nskipped==1?stest[0]:stest[1]); // Taken from Dominique Thiebaut
			}
			printf(">+------------------------------+\n");
			printf("\n--|>\n");
		}
